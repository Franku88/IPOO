    ...
    class Empresa
    {
        ...
        */
        ...
        public function buscarViaje($codViaje) {
            //Metodo que retorna el objeto Viaje con $codViaje de $colViajes
            //Retorna null si no es encontrado
            $encontrado = false;
            $viajes = $this->getColViajes();
            $limite = count($viajes);
            $i = 0;
            while (!$encontrado && $i < $limite) {
                $encontrado = $viajes[$i]->getNumero() == $codViaje;
                $i++;
            }
            if ($encontrado) {
                $retorno = $viajes[$i-1];
        ...
        public function darCostoViaje($codViaje) {
            //Metodo que retorna el costo de un viaje con $codViaje
            $viaje = $this->buscarViaje($codViaje);
            //Si no encuentra un viaje con $codViaje, retorna un costo < 0
            if ($viaje != null) {
                $costo = $viaje->calcularImporteViaje();
            } else {
                $costo = -1;
            }
            return $costo;
