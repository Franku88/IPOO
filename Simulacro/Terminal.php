<?php
    class Terminal
    {
        ...
        public function darViajeMenorValor() {
            //Metodo que retorna una coleccion del viaje mas barato de cada empresa
            $viajesBaratos = [];
            $empresas = $this->getColEmpresas();
            //Para cada empresa en $colEmpresas
            foreach($empresas as $empresa) {
                $viajes = $empresa->getColViajes();
                $lim = count($viajes);
                $viajeBarato = $viajes[0];
                //Recorro y comparo todos los viajes de $empresa
                for ($i = 1; $i < $lim; $i++) {
                    $viajeActual = $viajes[$i];
                    //Asigno como $viajeBarato al que menor importe tenga
                    if ($viajeBarato->getImporte() > $viajeActual->getImporte()) {
                        $viajeBarato = $viajeActual;
                    }
                }
                //Agrego viaje mas barato encontrado a la coleccion
                $viajesBaratos[] = $viajeBarato;
            }
